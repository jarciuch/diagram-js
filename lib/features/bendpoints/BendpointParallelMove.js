...
function BendpointParallelMove(injector, eventBus, canvas, dragging, graphicsFactory, rules, modeling) {
  ...
  eventBus.on('intersection.move.start', 1001, function(e) {
    ...
    //TODO: is it allowed and a good idea to do that?
    context.waypoints[0].original = Layout.getMid(source);
    context.waypoints[connection.waypoints.length - 1].original = Layout.getMid(target);
  ...
  eventBus.on('intersection.move.move', function(e) {
    ...
    if( p0 === waypoints[0] ) {
      ...
      if( lostConnection(p0, source) ) {
        ...
        p0[otherAxis] = source.center[otherAxis];
      }
    ...
    if( p1 === waypoints[idxLast()]) {
      ...
      if( lostConnection(p1, target) ) {
        ...
        p1[otherAxis] = target.center[otherAxis];
      }
    ...
    if (connectionDocking) {
      waypoints = connectionDocking.getCroppedWaypoints(context.connection, source, target);
    }
...
BendpointParallelMove.$inject = [ 'injector', 'eventBus', 'canvas', 'dragging', 'graphicsFactory', 'rules', 'modeling' ];
