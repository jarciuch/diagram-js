 ...
 */
function Bendpoints(injector, eventBus, canvas, interactionEvents, bendpointMove, connectionSegmentMove) {

  var self = this;

  self.moveConnectionSegment = function(event, connection, intersection) {
    connectionSegmentMove.start(event, connection, intersection.index);
  };

  self.moveBendpoint = function(event, connection, intersection) {
  ...
  function activateBendpointMove(event, connection) {
    ...
    if (isIntersectionMiddle(intersection, waypoints, 10)) {
      self.moveConnectionSegment(event, connection, intersection);
    } else {
      self.moveBendpoint(event, connection, intersection);
    }
  }
...
Bendpoints.$inject = [
  ...
  'interactionEvents', 'bendpointMove','connectionSegmentMove'
];
